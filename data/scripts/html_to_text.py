...
conference_name = "msr-2022"
html_code_filepath = f'./data/scripts/{conference_name}_raw_html.html'
...
for idx, tag in enumerate(appended_tags):
    ...
    time_info = soup2.find('strong').get_text(strip=True).replace('atMeeting', 'at Meeting').replace('atOffsite', 'at Offsite').replace("atMSR", "at MSR").replace("atRoom", "at Room")
...
output_file = open(f"./data/scripts/{conference_name}.json", 'w')
